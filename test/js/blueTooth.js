 ...
 * time                 2016-07-22
 * title                蓝牙体验插件
 * desc(介绍)           提供了一个全局的“blueTooth”方法，处理设备检测数据，优化蓝牙体验：
...
var blueTooth = (function () {
	...
	function 生成消息对照表() { // 进入controller只执行一次
		...
		// console.log("111 blueTooth.js 生成“对照表”", 对照表);
		return 对照表;
	};
	var 消息对照表 = 生成消息对照表();
	...
	return {
		...
		setApi : function (DevName, StatusName, Callback) {
			...
			if (res) {
				...
				return [res, "“消息对照表['" + DevName + "']”（消息对照项）存在，回调函数写入成功！"];
			} else {
				return [res, "“消息对照表['" + DevName + "']”（消息对照项）不存在，回调函数写入失败！"];
			}
		}
	};
})();
